...
class Administrador extends Component {
  ...
  toggleisActiveCursos = (ID) => {
    fetch(
      "http://localhost/App_v2/AcademiaFormaci%C3%B3n_V2/TASKS/coe-updateState.php?updateStateCursos=" +
        ID
    )
      .then((response) => response.json())
      .then((dataResponse) => {
  ...
  toggleisActiveRamos = (ID) => {
    fetch(
      "http://localhost/App_v2/AcademiaFormaci%C3%B3n_V2/TASKS/coe-updateStateRamos.php?updateStateRamos=" +
        ID
    )
      .then((response) => response.json())
      .then((dataResponse) => {
  ...
  toggleisActiveRelator = (ID) => {
    fetch(
      "http://localhost/App_v2/AcademiaFormaci%C3%B3n_V2/TASKS/coe-updateStateRelator.php?updateStateRelator=" +
        ID
    )
      .then((response) => response.json())
      .then((dataResponse) => {
  ...
  toggleisActiveColaborador = (ID) => {
    fetch(
      "http://localhost/App_v2/AcademiaFormaci%C3%B3n_V2/TASKS/coe-updateStateColaborador.php?updateStateColaborador=" +
        ID
    )
      .then((response) => response.json())
      .then((dataResponse) => {
  ...
  toggleisActiveClientes = (ID) => {
    fetch(
      "http://localhost/App_v2/AcademiaFormaci%C3%B3n_V2/TASKS/coe-updateStateClientes.php?updateStateClientes=" +
        ID
    )
      .then((response) => response.json())
      .then((dataResponse) => {
  ...
  render() {
    const { loadedData, cursos, ramos, relatores, colaboradores, clientes } =
      this.state;
    const styleLoading = {
      position: "absolute",
      top: "50%",
        ...
        >
                ...
                <tbody>
                  {cursos.map((curso) => (
                    <tr key={curso.ID}>
                      ...
                      <td>{curso.date}</td>
                      <td onChange={() => this.toggleisActiveCursos(curso.ID)}>
                        <ToggleSwitch isActive={curso.isActive} />
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {ramos.map((ramo) => (
                    <tr key={ramo.ID}>
                      ...
                      <td>{ramo.date}</td>
                      <td onChange={() => this.toggleisActiveRamos(ramo.ID)}>
                        <ToggleSwitch isActive={ramo.isActive} />
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {relatores.map((relator) => (
                    <tr key={relator.ID}>
                      ...
                      <td>{relator.date}</td>
                      <td
                        onChange={() => this.toggleisActiveRelator(relator.ID)}
                      >
                        <ToggleSwitch isActive={relator.isActive} />
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="card">
            <div className="card-body">
              ...
              <table
                id="tabla-cursos"
                className="table table-striped table-inverse tale-responsive"
              >
                ...
                <tbody>
                  {colaboradores.map((colaborador) => (
                    <tr key={colaborador.ID}>
                      ...
                      <td
                        onChange={() =>
                          this.toggleisActiveColaborador(colaborador.ID)
                        }
                      >
                        <ToggleSwitch isActive={colaborador.isActive} />
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {clientes.map((cliente) => (
                    <tr key={cliente.ID}>
                      ...
                      <td>{cliente.date}</td>
                      <td
                        onChange={() => this.toggleisActiveClientes(cliente.ID)}
                      >
                        <ToggleSwitch isActive={cliente.isActive} />
                      </td>
                    </tr>
                  ))}
                </tbody>
